...
#
# Usage:
#   bash experiments/tune_objectives.sh [MODE] [N_WORKERS] [N_TRIALS] [TUNE_EPOCHS]
#
...
#     outputs/tune/objectives/launcher.log \
#     experiments/tune_objectives.sh both 4 24 20
...
LOG_ROOT="${LOG_ROOT:-outputs/tune/objectives}"
mkdir -p "${LOG_ROOT}"

run_wdist() {
  echo "=== W-Dist objective tune ==="
  OUT_BASE="${OUT_BASE:-outputs/tune/wdist}" \
    bash experiments/tune_wdist_parallel.sh \
...
run_mcc() {
  ...
  OUT_BASE="${OUT_BASE:-outputs/tune/mcc_dbscan_${DBSCAN_BACKEND}}" \
    bash experiments/tune_mcc_parallel.sh \
...
echo "  W-Dist best: outputs/tune/wdist/best_wdist_${BACKEND}.json"
echo "  MCC best:    outputs/tune/mcc_dbscan_${DBSCAN_BACKEND}/best_mcc_${BACKEND}_dbscan_${DBSCAN_BACKEND}.json"
